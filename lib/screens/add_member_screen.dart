    ...
    return Scaffold(
      ...
      body: _isLoading
        ...
        children: [
          ...
          Expanded(
              child: ListView(
                children: temp.map((e) => MemberRectangle(memberName: e, update: () => _update())).toList(),
              )
          ),
        ],
      ),
                    ...
                    if (_selectedUserNames.length != 0){
                      await groups.where('groupName',isEqualTo: widget.groupName).get()
                        ...
                        value.docs.forEach((element) {
                          ...
                          addStatus(_selectedUserNames).forEach((key, value) {
                            temp2.update(key, (val) => true,ifAbsent: ()=>true);
                          });
                          _selectedUserNames.forEach((selectedUser) {
                            temp1.add(selectedUser);
                          });
                          groups.doc(element.id).update({'users' : temp1,'Status':temp2});
                        })
                      });
